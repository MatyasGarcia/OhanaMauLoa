...
export function ProductosProvider({ children }) {
    ...
    function obtenerProductos() {
        return(
            new Promise((res, rej) => {
                fetch('https://6815894f32debfe95dbbc52c.mockapi.io/productos')
                    .then((respuesta) =>
                        respuesta.json()
                    )
                    .then((datos) => {
    ...
    const agregarProducto = (producto) => {
        return(
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://6815894f32debfe95dbbc52c.mockapi.io/productos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto){
        return new Promise(async (res, rej) => {
            try {
                const respuesta = await fetch(`https://6815894f32debfe95dbbc52c.mockapi.io/productos/${producto.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const eliminarProducto = async (id) => {
        ...
        if (confirmar) {
            return (
                new Promise( async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://6815894f32debfe95dbbc52c.mockapi.io/productos/${id}`, {
                        method: 'DELETE',
                        });
